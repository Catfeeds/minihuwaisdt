...
class ShangchangController extends PublicController {
    ...
    public function index(){
    	...
    	//获取总页数
    	$eachpage=6;
        $count = M('shangchang')->where($condition)->count();
        $the_page = ceil($count/6);
    	//获取所有的商家数据
    	$store_list = M('shangchang')->where($condition)->order('sort desc,type desc')->field('id,name,uname,logo,tel,sheng,city,quyu')->limit($page.',6')->select();
    	...
    	echo json_encode(array('status'=>1,'store_list'=>$store_list,'eachpage'=>$eachpage));
    	exit();
    }
